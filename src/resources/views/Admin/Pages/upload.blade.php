<section id="main-list-data">
    ...
    <form method="post" action="{{ url('/admin/upload/' . $thisdata->pageConf->pageFatherId . '/'. $thisdata->pageConf->idRegister .'/save') }}" enctype="multipart/form-data" class="dropzone" id="my-dropzone">
        {{ csrf_field() }}
...
<section id="main-list-images">
   ...
   <h2 class="title-upload">Arquivos Carregados</h2>
    ...
    <ul id="main-list-files">
    @foreach($thisdata->uploadList as $upload)
        <li data-select="false">
            <img src="{{ $upload->imageFolder }}" />

            <div class="menu-image" data-id="{{ $upload->id_upload }}" data-link="{{ $upload->imageFolder }}" data-active="true">
