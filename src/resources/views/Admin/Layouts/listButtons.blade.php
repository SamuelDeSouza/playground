<section id="main-button-list">
    ...
    @endif
    @if(Auth::user()->listPermission[0]->add == 1)<a href="{{ url('/admin/'. $thisdata->pageConf->pageData->link . '/insert') }}{!! isset($thisdata->idFather) ? '/' . $thisdata->idFather : '' !!}" id="button-insert" class="main-button-list">INSERIR NOVO</a> @endif
    @if(Auth::user()->id_permission == 1)<a href="{{ url('/admin/'. $thisdata->pageConf->pageData->link . '/lixeira') }}{!! isset($thisdata->idFather) ? '/' . $thisdata->idFather : '' !!}" id="button-trash" class="main-button-list">LIXEIRA</a> @endif
</section>
