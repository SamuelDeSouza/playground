...
class SearchController extends Controller
{
    ...
    public function search($model_name, $father_id = null,Request $request){
      ...
      $model_table = with(new $this->_model)->getTable();

      // Retrieve all coluns from the model table
      $model_coluns = Schema::getColumnListing($model_table);
      
      // Create a query to search for the term
      $query = $this->_model::query();
      // One where for each column
      foreach($model_coluns as $column){
